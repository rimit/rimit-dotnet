...
namespace Rimit.Controllers
{
    ...
    public class TransactionDebitController : ApiController
    {
        private Config config = new Config();

        [Route("Debit/{tenant_id}")]
        [HttpPost]
        public IHttpActionResult Debit([FromUri()] int tenant_id, [FromBody()] RequestModel req)
        {
            Rimit.Utility.Response res = new Rimit.Utility.Response();

            Head head = new Head();
            head.apiVersion = "debitAmount";
            head.api = "v1";
