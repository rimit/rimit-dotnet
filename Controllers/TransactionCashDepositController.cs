...
namespace Rimit.Controllers
{
    ...
    public class TransactionCashDepositController : ApiController
    {
        private Config config = new Config();

        [Route("TransactionCashDeposit/{tenant_id}")]
        [HttpPost]
        public IHttpActionResult TransactionCashDeposit([FromUri()] int tenant_id, [FromBody()] RequestModel req)
        {
        ...
        [HttpPost]
        public IHttpActionResult WithdrawalSuccess( [FromBody()] TransactionWithdrawalModel req)
        {
            Rimit.Utility.Response res = new Rimit.Utility.Response();

            Head head = new Head();
            head.apiVersion = "confirmDebit";
            head.api = "v1";
            head.timeStamp = DateTime.Now.ToString("YYYY-MM-dd hh:mm:ss A");
            try
            {
