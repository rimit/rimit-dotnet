...
namespace Rimit.Controllers
{
    ...
    public class TransactionCashWithdrawalController : ApiController
    {
        private Config config = new Config();

        [Route("TransactionCashWithdrawal/{tenant_id}")]
        [HttpPost]
        public IHttpActionResult TransactionCashWithdrawal([FromUri()] int tenant_id, [FromBody()] RequestModel req)
        {
            Rimit.Utility.Response res = new Rimit.Utility.Response();

            Head head = new Head();
            head.apiVersion = "withdrawAmount";
            head.api = "v1";
        ...
        [Route("TransactionDeposit")]
        [HttpPost]
        public IHttpActionResult DepositSuccess( [FromBody()] TransactionWithdrawalModel req)
        {
            Rimit.Utility.Response res = new Rimit.Utility.Response();
            ...
            try
            {
                ...
                var DEBIT_CONFIRM_RESULT = new JavaScriptSerializer().Serialize(results);
                var _head = new JavaScriptSerializer().Serialize(DEBIT_CONFIRM_HEAD);
                var _dEBIT_CONFIRM_DATA = new JavaScriptSerializer().Serialize(dEBIT_CONFIRM_DATA);
